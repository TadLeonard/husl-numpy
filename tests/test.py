...
def test_to_hue_vs_old():
    ...
    for row in range(img.shape[0]):
        for col in range(img.shape[1]):
            ...
            _diff(hue_new[row, col], husl_old[0], diff=diff)
...
def test_lch_to_luv():
    ...
    _diff(lch_2, lch)  # just a sanity check on RGB -> LCH
    _diff(luv, luv_2)
